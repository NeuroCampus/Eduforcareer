...
const programmes = [
  { name: "Job Guaranteed Courses", href: "/#recent-courses" },
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          ...
          <div className="relative group">
            ...
            <div className="invisible absolute left-0 top-full z-50 min-w-[220px] rounded-lg bg-background py-2 opacity-0 shadow-lg transition-all group-hover:visible group-hover:opacity-100">
              {programmes.map((p) => (
                <a key={p.name} href={p.href} className="block px-4 py-2 text-sm text-foreground hover:bg-accent hover:text-accent-foreground">
                  {p.name}
                </a>
              ))}
            </div>
        ...
        <div className="border-t bg-background px-4 py-4 lg:hidden">
          <div className="flex flex-col gap-3">
            ...
            {progOpen && programmes.map((p) => (
              <a key={p.name} href={p.href} className="pl-4 text-sm text-muted-foreground">{p.name}</a>
            ))}
